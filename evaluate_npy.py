...
def do_python_eval(predict_folder, gt_folder, name_list, num_cls=21, input_type='png', threshold=1.0, printlog=False,
    ...
    for i in range(num_process):
        p = multiprocessing.Process(target=compare, args=(i, num_process, TP, P, T, input_type, threshold))
        p.start()
        p_list.append(p)
    for p in p_list:
        p.join()
...
def write_log(filepath, metric, comment):
    filepath = filepath
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datalist', type=str, default='data_ids/train.txt')
    parser.add_argument('--pred', type=str)
    parser.add_argument("--gt", type=str)
    parser.add_argument('--log', default='./evallog.txt', type=str)
    ...
    if not args.curve:
        loglist = do_python_eval(args.pred, args.gt, name_list, 21, args.type, args.t, printlog=True,
                                 num_process=args.num_process)
        write_log(args.log, loglist, args.comment)
    else:
        l = []
        for i in range(15, 35):
            t = i / 100.0
            loglist = do_python_eval(args.pred, args.gt, name_list, 21, args.type, t,
                                     num_process=args.num_process)
            l.append(loglist['mIoU'])
            print('%d/60 background score: %.3f\tmIoU: %.3f%%' % (i, t, loglist['mIoU']))
        write_log(args.log, {'mIoU': l}, args.comment)
